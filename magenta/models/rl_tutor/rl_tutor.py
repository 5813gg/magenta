...
class RLTutor(object):
  ...
  def training_step(self):
    ...
    if self.num_times_train_called % self.dqn_hparams.train_every_nth == 0:
      ...
      for i, (o, a, r, new_o) in enumerate(samples):
        observations[i, :len(o), :] = rl_tutor_ops.make_onehot(o, self.input_size)
        new_observations[i, :len(new_o), :] = rl_tutor_ops.make_onehot(new_o, self.input_size)
        action_mask[i, :] = a
        rewards[i] = r
        obs_lengths[i] = len(o)
  ...
  def collect_reward(self, obs, action, reward_scores, verbose=False):
    ...
    Returns:
      Float reward value.
    """
    if verbose: print "In RL Tutor collect reward"
  ...
  def generate_sample(self, title='rltutor_sample', visualize_probs=False, 
    prob_image_name=None, length=120, most_probable=False):
    """Generates a sample sequence with the current model.

    The sequence is generated by sampling from the output probabilities at each
    timestep, and feeding the resulting token back in as input to the model.
