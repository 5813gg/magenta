...
def make_cell(hparams, note_rnn_type, state_is_tuple=False):
  ...
  for num_units in hparams.rnn_layer_sizes:
    if note_rnn_type == 'default':
      cell = tf.contrib.rnn_cell.LSTMCell(
          num_units, state_is_tuple=state_is_tuple)
    else:
      cell = tf.contrib.rnn_cell.BasicLSTMCell(
          num_units, state_is_tuple=state_is_tuple)
      cell = tf.contrib.rnn_cell.DropoutWrapper(
          cell, output_keep_prob=hparams.dropout_keep_prob)
    cells.append(cell)

  cell = tf.contrib.rnn_cell.MultiRNNCell(cells, state_is_tuple=state_is_tuple)
