...
def augment_strokes(strokes, prob=0.0, discrete=False, relative=True):
  ...
  for i in range(len(strokes)):
    candidate = [strokes[i][0], strokes[i][1], strokes[i][2]] # candidate is the actual stroke
    ...
    if candidate[2] == 0 and prev_stroke[2] == 0 and count > 2 and urnd < prob:
      if discrete and not relative:
        continue # just drop if using absolute discrete values
      else:
        stroke[0] += candidate[0] # add this to the last stroke
        stroke[1] += candidate[1]
    else:
      stroke = candidate # normal stroke gets used
      prev_stroke = stroke
...
class DataLoader(object):
  ...
  def __init__(self,
    ...
    self.start_stroke_token = [0, 0, 0, 1, 0]  # S_0 in sketch-rnn paper # used to be
  ...
  def preprocess(self, strokes):
    ...
    for i in range(len(strokes)):
      ...
      if len(data) <= (self.max_seq_length):
        ...
        if self.discrete:
          data = convert_absolute_strokes_to_relative(data)
  ...
  def _get_batch_from_indices(self, indices):
    ...
    if self.discrete:
      x_batch = quantize_and_clip(x_batch)
...
def quantize_and_clip(x_batch):
  new_batch = []
  for x in x_batch:
    batch = np.clip(x, -128, 128)
    new_batch.append(np.array(batch).astype(np.int32))
  return new_batch
...
def normal_pdf(x, mu, sigma=2.0):
  return np.exp(-0.5 * (x - mu)**2 / sigma**2) / ((2 * np.pi * sigma**2)**0.5)

def smooth_labels(strokes, seq_lens, label_idx=0):
  smoothed_labels = []
  for i,sketch in enumerate(batch):
    seq_len = seq_lens[i]
    label_vect = [[normal_pdf(x, sketch[mu_idx,label_idx]) for x in range(256)] for mu_idx in range(seq_len)]
    smoothed_labels.append(label_vect)
