"""Discrete version of the sketch-RNN VAE Model.

The mean and stdev of the gaussians are output as discrete values. Would need
to take the argmax to pick the actual mean, aka break the gradient. Therefore
would need to train with RL.
"""
...
class Model(object):
  ...
  def build_model(self, hps):
    ...
    self.n_out = (3 + self.num_mixture + self.num_mixture * (self.hps.num_output_buckets * 5))

    with tf.variable_scope('RNN'):
      self.output_w = tf.get_variable('output_w', [self.hps.dec_rnn_size, self.n_out])
      self.output_b = tf.get_variable('output_b', [self.n_out])
    ...
    self.output = tf.nn.xw_plus_b(output, self.output_w, self.output_b)
    ...
    def get_mixture_coef(output):
      ...
      z_pi = z[:, 3:self.num_mixture+3]
      self.z_mu1, self.z_mu2, self.z_sigma1, self.z_sigma2, self.z_corr = tf.split(z[:, 3+self.num_mixture:], 5, 1)

      # take the bucketed continuous variables and put them back down to 1D
      self.z_mu1 = tf.reshape(self.z_mu1, [-1, self.num_mixture, self.hps.num_output_buckets])
      self.z_mu1 = tf.cast(tf.argmax(self.z_mu1, axis=2), tf.float32)
      self.z_mu1 = tf.scalar_mul((512.0/self.hps.num_output_buckets), self.z_mu1) - 256
      self.z_mu2 = tf.reshape(self.z_mu2, [-1, self.num_mixture, self.hps.num_output_buckets])
      self.z_mu2 = tf.cast(tf.argmax(self.z_mu2, axis=2), tf.float32)
      self.z_mu2 = tf.scalar_mul((512.0/self.hps.num_output_buckets), self.z_mu2) - 256

      #TODO(natashajaques): figure out good scaling parameters for sigmas
      self.z_sigma1 = tf.reshape(self.z_sigma1, [-1, self.num_mixture, self.hps.num_output_buckets])
      self.z_sigma1 = tf.cast(tf.argmax(self.z_sigma1, axis=2), tf.float32) / self.hps.sigma_scaler
      self.z_sigma2 = tf.reshape(self.z_sigma2, [-1, self.num_mixture, self.hps.num_output_buckets])
      self.z_sigma2 = tf.cast(tf.argmax(self.z_sigma2, axis=2), tf.float32) / self.hps.sigma_scaler

      self.z_corr = tf.reshape(self.z_corr, [-1, self.num_mixture, self.hps.num_output_buckets])
      self.z_corr = tf.cast(tf.argmax(self.z_corr, axis=2), tf.float32) / (self.hps.num_output_buckets / 2.0) - 1.0
      ...
      r = [z_pi, self.z_mu1, self.z_mu2, self.z_sigma1, self.z_sigma2, self.z_corr, z_pen, z_pen_logits]
      return r

    out = get_mixture_coef(self.output)
