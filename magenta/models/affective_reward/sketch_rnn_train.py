...
import sketch_rnn_model_discrete
import sketch_rnn_utils as utils
...
tf.app.flags.DEFINE_string(
    'data_dir',
    'TODO/path/sketch_rnn/data/discrete/',
...
tf.app.flags.DEFINE_boolean(
    'discrete', True,
    'Set to true to train the discrete version of sketch rnn')
tf.app.flags.DEFINE_string(
    'hparams', '',
...
tf.app.flags.DEFINE_boolean(
    ...
    'Set to true if reading data from remote directory.')
tf.app.flags.DEFINE_string(
    'master', '',
...
def load_dataset(data_dir, model_params, use_remote_data=False,
                 inference_mode=False, discrete=False):
  ...
  for dataset in datasets:
    ...
    if use_remote_data: # TODO: not really necessary unless using tf.gfile?
      tf.logging.info('Using remote data, path is: %s', data_filepath)
      input_file = tf.gfile.Open(data_filepath, 'r')
      data = np.load(input_file)
      tf.logging.info('Read remote data')
    elif data_dir.startswith('http://') or data_dir.startswith('https://'):
  ...
  valid_set = utils.DataLoader(
      ...
      discrete=discrete)
  if not discrete: valid_set.normalize(normalizing_scale_factor)
  ...
  test_set = utils.DataLoader(
      ...
      augment_stroke_prob=0.0,
      discrete=discrete)
  if not discrete: test_set.normalize(normalizing_scale_factor)
...
def train(sess, model, eval_model, train_set, valid_set, test_set, log_root):
  """Train a sketch-rnn model."""
  # Setup summary writer.
  summary_writer = tf.summary.FileWriter(log_root)
...
def trainer(model_params):
  ...
  datasets = load_dataset(FLAGS.data_dir, model_params, FLAGS.use_remote_data,
  ...
  if not FLAGS.discrete:
    ...
    model = sketch_rnn_model_discrete.Model(model_params)
    eval_model = sketch_rnn_model_discrete.Model(eval_model_params, reuse=True)
  ...
  if FLAGS.resume_training:
    ...
    tf.logging.info('Checkpoint successfully loaded from %s', FLAGS.log_root)
  ...
  tf.logging.info('Log directory created at %s', FLAGS.log_root)

  train(sess, model, eval_model, train_set, valid_set, test_set, FLAGS.log_root)


def main(unused_argv):
  """Load model params, save config file and start trainer."""
  if FLAGS.discrete:
    model_params = sketch_rnn_model_discrete.get_default_hparams()
  else:
    model_params = sketch_rnn_model.get_default_hparams()
  if FLAGS.hparams:
