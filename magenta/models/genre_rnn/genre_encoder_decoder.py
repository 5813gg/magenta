...
"""A MelodyEncoderDecoder specific to the GenreClassifer and GenreRNN models."""
...
class GenreMelodyEncoderDecoder(melodies_lib.MelodyEncoderDecoder):
  ...
  def __init__(self, genre, num_genres):
    """Initializes the MelodyEncoderDecoder.

    Args:
      genre: An integer representing the code for the genre of this melody.
      num_genres: The total number of genres that can be encoded.
    """
    super(GenreMelodyEncoderDecoder, self).__init__(MIN_NOTE, MAX_NOTE,
                                                    TRANSPOSE_TO_KEY)
    self.num_genres = num_genres
    self.genre = genre
    self._num_model_events = self.max_note - self.min_note + NUM_SPECIAL_EVENTS
    self.one_hot_length = self._num_model_events + self.num_genres
  ...
  def melody_to_input(self, melody):
    ...
    Genre is encoded using the bits at the end of the bit vector (bits after
    the class's _num_model_events
    ...
    """
    input_ = [0.0] * self.one_hot_length
    input_[self.melody_event_to_model_event(melody.events[-1])] = 1.0

    # encode genre at the end of the bit vector
    genre_index = self.one_hot_length - self.num_genres + self.genre
    input_[genre_index] = 1.0
